import"./modulepreload-polyfill-P2Xu9kJm.js";import"./main-US6YDRb4.js";import"./pwa-DYmKx9RI.js";/* empty css              */var e=(window.location.hostname===`localhost`||window.location.hostname,`/api`);document.addEventListener(`DOMContentLoaded`,async()=>{let t=document.getElementById(`blog-container`);if(t)try{let n=await fetch(`${e}/blogs?list=true`);if(!n.ok)throw Error(`Failed to load blogs`);let r=await n.json();if(r.length===0){t.innerHTML=`<div style="grid-column: 1/-1; text-align: center; padding: 50px;"><p style="color: var(--text-gray);">Belum ada artikel saat ini.</p></div>`;return}let i=3,a=()=>{let e=r.slice(0,i);if(t.innerHTML=`
            ...
            `+e.map(e=>{let t=e.createdAt?new Date(e.createdAt).toLocaleDateString(`id-ID`,{day:`numeric`,month:`short`,year:`numeric`}):``,n=e.coverImage&&e.coverImage.trim()!==``?e.coverImage:`https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?auto=format&fit=crop&q=80&w=600`;return`
